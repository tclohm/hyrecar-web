...
const CarDetail = () => {
	...
	if (data && data.car) {
		...
		const profileUrl = `/users/show/${data.car.owner.id}`
			...
			<Fragment>
				...
				<div className="fixed w-full sm:bottom-0 bottom-12 bg-white p-2 border border-b-0 sm:border flex justify-between items-center sm:text-sm text-xs">
					<div className="flex items-center ml-8">
						<Link to={profileUrl} className="h-12 w-12 md:h-16 md:w-16 bg-cover rounded-xl" style={{ backgroundImage: "url(" + profileImage + ")" }} />
						<div className="flex flex-col ml-4">
							<div className="flex">
								{stars}
							</div>
							<Link to={profileUrl} className="font-semibold">{data.car.owner.firstName} {data.car.owner.lastName}</Link>
						</div>
					</div>
					<div className="flex">
